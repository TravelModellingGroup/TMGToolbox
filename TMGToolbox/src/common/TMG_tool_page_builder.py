...
class TmgToolPageBuilder(_m.ToolPageBuilder):
    ...
    def add_select_output_matrix(self, tool_attribute_name,
        ...
        options = []
        if include_none:
            options.append((-1, "None - Do not save"))
            ...
            for matrixType in matrix_types:
                ...
                for i in range(1, dimension + 1):
                    ...
                    if matrix is None and include_new:
                        options.append((id, "%s - New matrix %s" % (id, id)))

                    elif matrix is not None and include_existing:
                        if matrix.read_only:
                            continue  # Skip protected matrices
                        options.append((id,
    ...
    def add_table(self, visible_border=False, title="", width=None):
        ...
        of layout tables inside the Page.
